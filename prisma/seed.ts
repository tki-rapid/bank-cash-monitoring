...
async function main() {
  const ceo = await prisma.user.upsert({ where: { email: "ceo@pt-tki.internal" }, update: {}, create: { email: "ceo@pt-tki.internal", name: "CEO PT TKI", role: AppRole.CEO } });
  const finance = await prisma.user.upsert({ where: { email: "finance@pt-tki.internal" }, update: {}, create: { email: "finance@pt-tki.internal", name: "Finance PT TKI", role: AppRole.FINANCE } });
  ...
  console.log(`Seeded ${ceo.name}, ${finance.name}, and ${bni.name} without deleting existing records.`);
}
